...
|---|---|---|
| `u3v-sdk-2.3.0-windows-x64.zip` | `bin/u3v_cam.dll` (+ `bin/libusb-1.0.dll`) | `lib/u3v_cam.lib`, `include/u3v/*.h` |
| `u3v-sdk-2.3.0-linux-x64.tar.gz` | `lib/libu3v_cam.so.2.3.0` (+ SONAME symlinks) | `include/u3v/*.h` |
| `u3v-sdk-2.3.0-linux-arm64.tar.gz` | Same layout as x64 | Same |
| `u3v-sdk-2.3.0-macos-arm64.zip` | `lib/libu3v_cam.2.3.0.dylib` (+ symlinks) | `include/u3v/*.h` |
| `u3v-sdk-2.3.0-macos-x64.zip` | Same layout as arm64 | Same |
| `u3v-sdk-2.3.0-python.zip` | Native libraries for all five platforms bundled inside | Not required — Python wraps them |
...
```
libu3v_cam.so.2.3.0
libu3v_cam.so.2          (symlink to libu3v_cam.so.2.3.0)
plugins/                 (optional)
```
...
```bash
pip install u3v-sdk-2.3.0-python.zip
```
...
runs against 2.3.0 without recompile; drop the new library file in
