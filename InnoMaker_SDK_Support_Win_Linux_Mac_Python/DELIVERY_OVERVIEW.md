...
|---|---|---|---|
| `u3v-sdk-2.2.3-windows-x64.zip`   | Windows 10/11 x64 | 18 MB | C/C++ SDK + Qt6 viewer, fully self-contained |
| `u3v-sdk-2.2.3-linux-x64.tar.gz`  | Ubuntu 22.04+ x64 | 150 KB | C/C++ SDK + viewer (system Qt6 + libusb) |
| `u3v-sdk-2.2.3-linux-arm64.tar.gz`| Pi 5 / Jetson Orin Nano | 150 KB | ARM64 counterpart of the Linux x64 package |
| `u3v-sdk-2.2.3-macos-arm64.zip`   | Apple Silicon (M1–M4) | 55 MB | C/C++ SDK + `u3v_viewer.app` |
| `u3v-sdk-2.2.3-macos-x64.zip`     | Intel Mac | 55 MB | Intel counterpart of the macOS arm64 package |
| `u3v-sdk-2.2.3-python.zip`        | All five OS/arch above | 1.1 MB | Python 3.8+ package; bundles native libraries for every platform in one archive |
...
(e.g. `u3v-sdk-2.2.3-windows-x64/`), so multiple platforms can coexist
...
## 3. Windows Package (`u3v-sdk-2.2.3-windows-x64.zip`)
...
```
1. Extract u3v-sdk-2.2.3-windows-x64.zip to any directory
...
## 4. Linux Package (`u3v-sdk-2.2.3-linux-x64.tar.gz` / `-linux-arm64.tar.gz`)
...
│   ├── libu3v_cam.so.2           → libu3v_cam.so.2.2.3
│   └── libu3v_cam.so.2.2.3       Actual library
├── include/u3v/*.h               Public API headers (7 files)
├── examples/
│   └── basic_capture.c           Compilable reference source
├── docs/
...
tar xzf u3v-sdk-2.2.3-linux-x64.tar.gz          # x86_64 PC / Docker
# or
tar xzf u3v-sdk-2.2.3-linux-arm64.tar.gz        # Pi 5 / Jetson
cd u3v-sdk-2.2.3-linux-*
...
## 5. macOS Packages (`u3v-sdk-2.2.3-macos-arm64.zip` / `-macos-x64.zip`)
...
│   ├── libu3v_cam.2.dylib        → libu3v_cam.2.2.3.dylib
│   └── libu3v_cam.2.2.3.dylib    Actual library
├── include/u3v/*.h               Public API headers (7 files)
...
unzip u3v-sdk-2.2.3-macos-arm64.zip     # Apple Silicon
# or
unzip u3v-sdk-2.2.3-macos-x64.zip       # Intel

cd u3v-sdk-2.2.3-macos-*
...
## 6. Python Package (`u3v-sdk-2.2.3-python.zip`)
...
**Windows:**
```bat
:: 1. Extract u3v-sdk-2.2.3-python.zip
...
**Linux / macOS:**
```bash
unzip u3v-sdk-2.2.3-python.zip
cd u3v-sdk-2.2.3-python
chmod +x *.sh
./install_deps.sh
...
| Application binary compatibility | 2.1.x / 2.2.x code links and runs against 2.2.3 without recompile (SONAME libu3v_cam.so.2 unchanged) |
...
4. **Choosing a package**
   - Desktop applications, system integrators →
     `u3v-sdk-2.2.3-windows-x64.zip` and/or
     `u3v-sdk-2.2.3-linux-<arch>.tar.gz` and/or
     `u3v-sdk-2.2.3-macos-<arch>.zip`
   - Python / ML / CV teams, headless capture, scripting →
     `u3v-sdk-2.2.3-python.zip` (single file covers all five platforms)
