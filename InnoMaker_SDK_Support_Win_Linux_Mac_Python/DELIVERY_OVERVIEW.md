...
| `u3v-sdk-2.2.0-python.zip`        | All five OS/arch above | 1.1 MB | Python 3.8+ package; bundles native libraries for every platform in one archive |
...
(e.g. `u3v-sdk-2.2.0-windows-x64/`), so multiple platforms can coexist
...
sensor-specific values differ.

| Parameter | Sony IMX296 (Mono) | Sony IMX296 (Color) | OmniVision OV9281 | Sony IMX585 |
|---|---|---|---|---|
| Status | ✅ Production (2.2.0) | ✅ Production (2.2.0) | ✅ Production (2.2.0) | 🟡 Reserved |
| Native resolution | 1456 × 1088 | 1456 × 1088 | TBD | TBD |
| Pixel size | 3.45 µm | 3.45 µm | TBD | TBD |
| Shutter type | Global | Global | TBD | TBD |
| Max frame rate (full ROI) | 60 fps | 60 fps | TBD | TBD |
| Pixel formats | Mono8/10/12/16 | BayerGR8 (+ ISP → RGB8) | TBD | TBD |
| Gain register range | 0 – 480 | 0 – 480 | TBD | TBD |
| Hardware trigger | ✅ Verified (line0–3 / software, rising / falling) | ✅ | TBD | TBD |
| Verified hardware | Pi 5, Jetson Orin Nano, Win 10/11 x64, macOS 12+ | Same | Same | — |
...
## 3. Windows Package (`u3v-sdk-2.2.0-windows-x64.zip`)
...
```
1. Extract u3v-sdk-2.2.0-windows-x64.zip to any directory
2. Plug in the U3V camera
3. If the device isn't recognized, install the WinUSB driver
   (see the WinUSB installation guide shipped with prior packages,
    or contact us for the Zadig-based installer)
...
## 4. Linux Package (`u3v-sdk-2.2.0-linux-x64.tar.gz` / `-linux-arm64.tar.gz`)
...
│   ├── libu3v_cam.so.2           → libu3v_cam.so.2.2.0
│   └── libu3v_cam.so.2.2.0       Actual library
├── include/u3v/*.h               Public API headers (7 files)
├── examples/
│   └── basic_capture.c           Compilable reference source
├── docs/
...
tar xzf u3v-sdk-2.2.0-linux-x64.tar.gz          # x86_64 PC / Docker
# or
tar xzf u3v-sdk-2.2.0-linux-arm64.tar.gz        # Pi 5 / Jetson
...
## 5. macOS Packages (`u3v-sdk-2.2.0-macos-arm64.zip` / `-macos-x64.zip`)
...
│   ├── libu3v_cam.2.dylib        → libu3v_cam.2.2.0.dylib
│   └── libu3v_cam.2.2.0.dylib    Actual library
├── include/u3v/*.h               Public API headers (7 files)
...
unzip u3v-sdk-2.2.0-macos-arm64.zip     # Apple Silicon
# or
unzip u3v-sdk-2.2.0-macos-x64.zip       # Intel
...
## 6. Python Package (`u3v-sdk-2.2.0-python.zip`)
...
Color-variant sensors (e.g. IMX296 Color) output Bayer-pattern frames
that need demosaic + optional white balance / gamma / color correction
before they look right on a display or feed into downstream vision code.
...
Mono workflows written against 2.1.x continue to work in 2.2.0 without
modification.
...
| Application binary compatibility | 2.1.x code links and runs against 2.2.0 without recompile |
...
4. **Choosing a package**
   - Desktop applications, system integrators →
     `u3v-sdk-2.2.0-windows-x64.zip` and/or
     `u3v-sdk-2.2.0-linux-<arch>.tar.gz` and/or
     `u3v-sdk-2.2.0-macos-<arch>.zip`
   - Python / ML / CV teams, headless capture, scripting →
     `u3v-sdk-2.2.0-python.zip` (single file covers all five platforms)
